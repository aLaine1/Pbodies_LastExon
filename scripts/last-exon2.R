...
for (j in names(NormalizedList)){
  if (length(which(is.na(NormalizedList[[j]])))==0){
    filteredlist[[j]]<-NormalizedList[[j]]
...
###  Generate output  ###
write.table(exportTable4,paste0(args$output, "/Final_Last_Exon.tsv",collapse = ""),sep="\t",row.names = F,quote=F)
